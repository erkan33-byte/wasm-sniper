...
async function loadWasm() {
    // We laden het specifieke WASM bestand dat we hebben gegenereerd
...
// Hulpmiddelen voor WIF generatie
...
async function generateWIF(privHex) {
    const payload = "80" + privHex + "01"; // Compressed
...
self.onmessage = async function(ev) {
    try {
        if (ev.data.type === "ADDRESS_SCAN") {
            ...
            const { startHex, targetH160Hex, workerId } = ev.data;
            const startKey = BigInt("0x" + startHex);
            const targetH160 = new Uint8Array(targetH160Hex.match(/.{2}/g).map(b => parseInt(b, 16)));

            const memory = wasmInstance.exports.memory;
            const startKeyPtr = 1024;
            const targetH160Ptr = 1060;
            const foundOffsetPtr = 1100;
            ...
            while (true) {
                ...
                // Korte pauze voor browser responsiviteit elke 100k keys
                if (currentKey % 100000n === 0n) {
