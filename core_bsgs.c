...
static fe fpow(fe a){return fpowexp(a,PM2);}
...
int bsgs_scan(const uint8_t pub[33],const uint8_t start[32],const uint8_t end[32],uint32_t m,uint32_t giants,uint8_t out[32]){
 if(cmpbe(start,end)>0||!m||!giants)return -1;if(bsgs_prepare(m))return -2;aff target;if(!decompress(pub,&target))return -3;
